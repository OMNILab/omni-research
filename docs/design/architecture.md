...
- **Produced by**: `omr-analyze` (research-brief, evidence-map, judgment-summary, research-plan), `omr-decision` (architecture-decision), `omr-evaluation` (experiment-spec, evaluation-report)
...
- **Generated by**: `omr-synthesis` (post-Gate-D internal step; skip with `--no-wiki`)
