...
class SearchCollector:
    ...
    def _search_google_scholar_via_mcp(
    ...
    def _search_arxiv_sdk(self, query: str, max_results: int) -> List[Dict]:
        ...
        try:
            ...
            # Configure client
            client = arxiv.Client(
                page_size=max_results, delay_seconds=3.0, num_retries=2
            )
    ...
    def collect_approved(self, search_results: Dict, selection: str) -> Dict:
        ...
        search_dir = (
            self.workspace_root / "materials" / "search" / f"query-{query_hash}"
        )
        ...
        query_metadata = {
            ...
            "results_per_source": {
                "google_scholar": {
                    "found": len(search_results["google_scholar"]),
                    "collected": 0,
                },
                "arxiv": {
                    "found": len(search_results["arxiv"]),
                    "collected": len(
                        [p for p in papers_to_download if p["source"] == "arxiv"]
                    ),
                },
                "github": {"found": len(search_results["github"]), "collected": 0},
                "huggingface": {
                    "found": len(search_results["huggingface"]),
                    "collected": 0,
                },
            },
        ...
        # Update search index
        search_index = (
            self.workspace_root / "docs" / "index" / "search-queries-index.json"
        )
...
def main():
    ...
    if importlib.util.find_spec("arxiv") is not None:
        print("Using arxiv SDK (enhanced search)")
    else:
        print("Using arxiv API fallback (SDK not installed)")
    ...
    try:
        ...
        if client.detect_server():
            print("Chrome MCP available (Google Scholar search)")
        else:
            print("Chrome MCP not available (arxiv-only search)")
    except ImportError:
        print("MCP SDK not installed (arxiv-only search)")
