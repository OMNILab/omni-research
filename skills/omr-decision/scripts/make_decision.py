    ...
    alternatives.append(
        {
            ...
            "evidence_support": [c["id"] for c in suggested_claims[:3]]
            if suggested_claims
            else [],
            "proven_support": len(
                [
                    ...
                    if "simple" in c["text"].lower() or "minimal" in c["text"].lower()
                ]
            ),
        }
    )
    ...
    alternatives.append(
        {
            ...
            "evidence_support": [c["id"] for c in proven_claims[:5]]
            if proven_claims
            else [],
    ...
    alternatives.append(
        {
            ...
            "evidence_support": [
                c["id"] for c in proven_claims[:3] + suggested_claims[:2]
            ]
            if proven_claims
            else [],
            "proven_support": len(proven_claims) // 2
            + len(suggested_claims) // 3,  # Moderate proven support
        }
    )
...
def select_alternative(alternatives: List[Dict], evidence_map: str) -> Dict:
    ...
    if proven_count >= 5:
        selected["rationale_details"] = (
            f"Selected based on strong proven evidence support ({proven_count} proven claims). High confidence in architecture choice."
        )
    elif proven_count >= 2:
        selected["rationale_details"] = (
            f"Selected based on moderate proven evidence support ({proven_count} proven claims). Validation recommended."
        )
    else:
        selected["rationale_details"] = (
            "Selected based on limited proven evidence. Additional validation required to confirm architecture."
        )
...
def generate_decision_doc(
    alternatives: List[Dict], selected: Dict, evidence_map_path: Path, judgment: str
) -> Dict:
    """
...
"""
    ...
    if judgment:
        markdown += "- Judgment Summary: `judgment-summary.md`\n"
    ...
    if proven_count < 3:
        ...
        markdown += (
            "  - **Mitigation**: Run validation experiments to confirm architecture\n\n"
        )

    markdown += "- **Risk 2**: Trade-offs may impact future scalability\n"
    markdown += (
        "  - **Mitigation**: Design modular components for incremental refinement\n\n"
    )
    ...
    markdown += (
        f"- Selected: {selected['name']} (Score: {selected.get('score', 'N/A')})\n"
    )
    markdown += (
        f"- Evidence foundation: {selected.get('proven_support', 0)} proven claims\n"
    )
...
def main():
    ...
    if result["status"] == "success":
        ...
        if result["gate_passed"]:
            print("\n⚠️  GATE B: Passed")
            print("  Architecture decision sound")

        print("\n📊 Skill tree updated")
        print("  - omr-evaluation [READY]")

    elif result["status"] == "gate_failed":
        print("⚠️  GATE B FAILED")
